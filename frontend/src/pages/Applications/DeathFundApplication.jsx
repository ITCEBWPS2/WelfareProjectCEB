...
const DeathFundForm = ({ onClose }) => { // Added onClose prop
  ...
  const [showSuggestions, setShowSuggestions] = useState(false); // Added for suggestion control
  ...
  const fetchMembers = async () => {
    try {
      ...
      toast.error("Error fetching members for EPF suggestions.");
    }
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "epf") {
      ...
      const suggestions = members.filter((member) =>
        member.epf.toString().toLowerCase().startsWith(lowerValue)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error submitting death fund form:", error);
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50 overflow-y-auto p-4">
      <div className="bg-white rounded-lg p-6 max-w-lg w-full shadow-lg relative my-8">
        ...
        <p className="text-gray-600 mb-6 text-center">
          Please fill this form to add a new death fund entry.
        </p>
