...
const RetirementGiftsTable = ({ tableTitle }) => {
    ...
    const fetchRetirementGifts = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${BASE_URL}/api/retirementgifts`, {
                withCredentials: true,
            });
            // Assuming your backend returns an array of retirement gift objects like:
            // [{ _id: "1", epfNumber: "12121", amount: 20000, date: "2025-04-28T00:00:00.000Z" }]
    ...
    const handleDelete = async (retirementGiftId) => {
        if (window.confirm("Are you sure you want to delete this retirement gift entry?")) {
            try {
                await axios.delete(`${BASE_URL}/api/retirementgifts/${retirementGiftId}`, {
                    withCredentials: true,
                });
                        ...
                        {retirementGifts.length === 0 ? (
                            <tr>
                                <td colSpan="4" className="text-center py-4 text-gray-500">
                                    {loading ? "Loading retirement gifts..." : "No retirement gifts found."}
                                </td>
                            </tr>
                        ) : (
                            retirementGifts.map((gift) => (
                                <tr key={gift._id} className="border-b border-gray-200 last:border-b-0 hover:bg-gray-100/70">
                                    ...
                                    <td className="px-4 py-2 text-sm whitespace-nowrap text-gray-800">
                                        {gift.date ? new Date(gift.date).toLocaleDateString("en-GB") : "N/A"}
                                    </td>
                                    <td className="px-4 py-2 text-sm whitespace-nowrap text-gray-800">
                                        {gift.amount}
                                    </td>
                                    <td className="px-4 py-2 text-sm whitespace-nowrap text-gray-800">
                                        <div className="flex space-x-2 items-center">
                                            {/* Edit/View Details for Retirement Gift Entry */}
                                            ...
                                            <button
                                                onClick={() => fetchMemberId(gift.epfNumber)}
                    ...
                    <DialogContent onClose={() => setShowRetirementGiftDetailsDialog(false)} className="max-w-xl">
                        <DialogHeader className="border-b pb-4 mb-4">
                            ...
                            <DialogDescription className="text-gray-600">
                                Details of the selected retirement gift entry.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="py-2 text-gray-700 space-y-3 text-sm grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                            <p><strong>EPF Number:</strong> {currentRetirementGiftDetails.epfNumber || "N/A"}</p>
...
const RetirementGiftsDashboard = () => {
    ...
    const handleNewRetirementGift = () => {
        navigate("/dashboard/retirementgifts/add");
    };
            ...
            >
                ...
                <div className="relative z-10 p-6 min-h-screen">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center mt-20 mb-6 space-y-4 md:space-y-0 md:space-x-4 max-w-7xl mx-auto">
                        <button
                            onClick={handleNewRetirementGift}
                    ...
                    {/* Retirement Gifts Table */}
                    <RetirementGiftsTable tableTitle={tableTitle} />
                </div>
            </main>
        </div>
    );
